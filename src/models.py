...
from src.GICB import GICB
from src.LICB import LICB
from src.dlutils import *
from src.constants import *
from src.pytorch_revgrad import RevGrad
...
class DITAU_Net(nn.Module):
    def __init__(self, feats):
        super(DITAU_Net, self).__init__()
        self.name = 'DITAU_Net'
        self.lr = lr
        self.batch = 128
        self.n_feats = feats
        self.n_window = 10
        self.LICB = LICB(in_features=feats, drop=0.2)
        self.GICB = GICB(num_inputs=self.n_window, num_outputs=feats, drop=0.2)
        ...
        self.transformer_encoder3 = TransformerEncoder(encoder_layers2, num_layers=1)
        self.transformer_encoder4 = TransformerEncoder(encoder_layers2, num_layers=1)
        ...
        self.decoder3 = nn.Sequential(RevGrad(), nn.Linear(self.n_window, 1), nn.Sigmoid())
        self.decoder4 = nn.Sequential(nn.Linear(self.n_window, self.n_window), nn.Sigmoid())
        self.decoder5 = nn.Sequential(nn.Linear(self.n_window, self.n_window), nn.Sigmoid())

    def callback(self, src, c):
        src2 = src + c
        g_atts = self.GICB(src2)
        src2 = g_atts.permute(2, 0, 1) * math.sqrt(self.n_feats)
        src2 = self.pos_encoder(src2)
        memory = self.transformer_encoder2(src2)
        return memory

    def forward(self, src, src_noise):
        l_atts_noise = self.LICB(src_noise)
        src3 = l_atts_noise.permute(2, 0, 1) * math.sqrt(self.n_feats)
        src3 = self.pos_encoder(src3)
        z3 = self.transformer_encoder1(src3)
        x3 = self.decoder3(z3.permute(1, 2, 0))

        l_atts = self.LICB(src)
        src1 = l_atts.permute(2, 0, 1) * math.sqrt(self.n_feats)
        src1 = self.pos_encoder(src1)
        z1 = self.transformer_encoder1(src1)
        x1 = self.decoder1(z1.permute(1, 2, 0))

        e2 = self.callback(src, x1)
        e3 = self.transformer_encoder3(e2)
        e4 = self.transformer_encoder4(e3)

        d2 = e2 + self.fcn(e2)
        d3 = e3 + self.fcn(e3)
        d4 = e4 + self.fcn(e4)

        x5 = self.decoder5(d4.permute(1, 2, 0))
        x4 = self.decoder4(d3.permute(1, 2, 0) + x5)
        x2 = self.decoder2(d2.permute(1, 2, 0) + x4)

        return x1.permute(0, 2, 1), x2.permute(0, 2, 1), x3.permute(0, 2, 1)  # (Batch, 1, output_channel)
